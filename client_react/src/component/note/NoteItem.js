...
const NoteItem = ({ projectId, note, onDelete, onUpdate }) => {
    ...
    const deleteNoteOnClick = async (id) => {
        ...
        const data = await remove(projectId, id)
        console.log('NoteItem delete', data)
    ...
    const upddateNoteFavorite = async () => {
        ...
        const data = await updateFavorited(projectId, note._id, { favorited: !isLiked } )
        onUpdate()
        if (data.success) {
    ...
    const editedNote = async () => {
        ...
        const data = await update(projectId, note._id, noteInfomation)
        setButtonDisabled(false)
        setIsEdit(!setIsEdit)
    };
